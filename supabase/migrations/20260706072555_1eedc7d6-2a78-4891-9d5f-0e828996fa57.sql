...
DECLARE
  ...
  scheduled_due_count integer := 0;
  live_due_count integer := 0;
  batch_id uuid;
...
BEGIN
  ...
  dur_sec := COALESCE(cfg.dur, 120);
  anim_sec := COALESCE(cfg.anim, 30);
  match_count := COALESCE(cfg.per_round, 5);
  max_market_score := COALESCE(cfg.market_score, 7);

  SELECT COALESCE(virtual_round_batch_id, virtual_round_id, id)
    ...
    FOR scheduled_row IN
      SELECT id
       ...
       WHERE is_virtual = true
         ...
         AND COALESCE(virtual_round_batch_id, virtual_round_id, id) = current_batch_id
         AND COALESCE(lock_time, now()) <= now()
  ...
  IF NOT COALESCE(cfg.running, false) THEN
  ...
  SELECT COUNT(*) FILTER (WHERE status = 'scheduled' AND COALESCE(lock_time, now()) <= now()),
         COUNT(*) FILTER (WHERE status = 'live' AND COALESCE(locked_at, lock_time, start_time, created_at, now()) + (anim_sec || ' seconds')::interval <= now())
    INTO scheduled_due_count, live_due_count
    FROM public.matches
   WHERE is_virtual = true
     AND status IN ('scheduled', 'live');
  ...
  RETURN jsonb_build_object(
    ...
    'live_due', live_due_count,
    'matches_per_round', match_count,
